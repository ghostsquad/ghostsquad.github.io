...
- [ ] Setting up a domain that points to the cluster, so that we don't have to update the ingress
- [ ] Setting up [external-dns](https://github.com/kubernetes-sigs/external-dns) to automatically update our domain with the list of IPs of hosts (when the get preempted)
